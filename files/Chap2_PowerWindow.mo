within ;
package Chap2_PowerWindow "Power window system with control logic"
  model Window
  ...
  model ScissorMechanism_Linear
    "Basic scissor mechanism with Modelica components"
    parameter Modelica.SIunits.Length Length_arm = 0.36
      "Longueur du bras (complet) - Arm length";
  ...
  end ScissorMechanism_Linear;

  model ScissorMechanism_NonLinear
    "Scissor mechanism with Modelica equations"
    parameter Modelica.SIunits.Length Length_arm = 0.36
      "Longueur du bras (complet) - Arm length";
  ...
  equation
    ...
    // lien effort lineaire / Couple, obtenu par travaux virtuels
    // Effort / Torque relation

    flange_b.s = Length_arm * sin(flange_a.phi);
    // lien positions lineaire / angle
    // Linear position / Angular Position relation
  ...
  end ScissorMechanism_NonLinear;

  model ScissorMechanism_withInertia
    "Scissor Mechanism (non linear) with inertia"
    parameter Modelica.SIunits.Length Length_arm = 0.36
      "Longueur du bras (complet) - Arm length";
    Modelica.Mechanics.Translational.Interfaces.Flange_b flange_b annotation(Placement(transformation(extent = {{-10, 88}, {10, 108}}), iconTransformation(extent = {{-10, 88}, {10, 108}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_a flange_a annotation(Placement(transformation(extent = {{-110, -70}, {-90, -50}}), iconTransformation(extent = {{-110, -70}, {-90, -50}})));
    ScissorMechanism_NonLinear mecanisme_Ciseau_Simple(Length_arm=Length_arm)
      annotation (Placement(transformation(extent={{-24,-30},{26,22}})));
  ...
  equation
    ...
    annotation(Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics={  Rectangle(extent = {{-68, 68}, {100, 52}}, lineColor = {0, 0, 255}, fillColor = {85, 170, 255},
              ...
              fillPattern =                                                                                                    FillPattern.Solid), Line(points = {{-60, -60}, {60, 60}}, color = {0, 0, 255}, smooth = Smooth.None, thickness = 1), Line(points = {{-60, 60}, {58, -60}}, color = {0, 0, 255}, smooth = Smooth.None, thickness = 1), Line(points = {{-92, -60}, {-66, -60}}, color = {0, 0, 0}, smooth = Smooth.None), Line(points = {{0, 68}, {0, 88}}, color = {0, 0, 0}, smooth = Smooth.None)}), Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics));
  end ScissorMechanism_withInertia;
  ...
  model DCmotor
    parameter Modelica.SIunits.Resistance Rmoteur = 0.35 "Resistance DC motor";
    parameter Modelica.SIunits.Inductance Lmoteur = 1e-5 "Inductance DC motor";
    parameter Modelica.SIunits.ElectricalTorqueConstant Kmoteur = 0.018
      "Electromechanic constant";
    parameter Modelica.SIunits.Inertia Jmoteur = 1.2e-5 "Inertia DC motor";
    Modelica.Electrical.Analog.Basic.EMF emf(k = Kmoteur) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}})));
  ...
  model PowerWindow_1stTest "The motor is equivalent to a torque source"
    Window vitre(F_friction=50)
      annotation (Placement(transformation(extent={{36,20},{82,60}})));
    ScissorMechanism_NonLinear mecanisme_Ciseau
      annotation (Placement(transformation(extent={{40,-26},{80,6}})));
  ...
  end PowerWindow_1stTest;

  model PowerWindow_NoStiffness
    Window vitre
      annotation (Placement(transformation(extent={{36,20},{82,60}})));
    ScissorMechanism_Linear mecanisme_Ciseau
      annotation (Placement(transformation(extent={{40,-26},{80,6}})));
  ...
  equation
    ...
    annotation(Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics));
  end PowerWindow_NoStiffness;

  model PowerWindow_full
    Window vitre(Mass_Vitre=3.2)
      annotation (Placement(transformation(extent={{85,14},{131,54}})));
    ScissorMechanism_NonLinear mecanisme_Ciseau
      annotation (Placement(transformation(extent={{88,-26},{128,6}})));
  ...
  model ControlLogic
    ...
    Window vitre(Mass_Vitre=3.2)
      annotation (Placement(transformation(extent={{137,-10},{183,30}})));
    ScissorMechanism_NonLinear mecanisme_Ciseau
      annotation (Placement(transformation(extent={{140,-50},{180,-18}})));
  ...
  model Logic_uC
    Window vitre(Mass_Vitre=3.2, Xinit=-0.2)
      annotation (Placement(transformation(extent={{145,-8},{191,32}})));
    ...
    Modelica.Mechanics.Rotational.Components.SpringDamper CompensationSpring(
      phi_rel0(displayUnit="rad") = 2.8,
      c=3,
      d=0.1) annotation (Placement(transformation(extent={{92,-90},{112,-70}})));
    ...
    ScissorMechanism_withInertia mecanisme_Ciseau_Inertie(J=2/0.3^2)
      annotation (Placement(transformation(extent={{146,-52},{192,-14}})));
  ...
  equation
    ...
    connect(fixed.flange, CompensationSpring.flange_a) annotation (Line(
        points={{88,-80},{92,-80}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(lossyGear.flange_a,DCmotor_. flange_b1) annotation(Line(points = {{40, -44}, {20, -44}}, color = {0, 0, 0}, smooth = Smooth.None));
    ...
    connect(relaySPDT.COM,DCmotor_. p1) annotation(Line(points = {{-24, -16.4}, {-12, -16.4}, {-12, -16}, {10, -16}, {10, -34}}, color = {0, 0, 255}, smooth = Smooth.None));
    ...
    connect(DCmotor_.n1, relaySPDT1.COM) annotation(Line(points = {{10, -54}, {-22, -54}, {-22, -40.4}, {-46, -40.4}}, color = {0, 0, 255}, smooth = Smooth.None));
    ...
    connect(CompensationSpring.flange_b, idealGear.flange_b) annotation (Line(
        points={{112,-80},{132,-80},{132,-44},{118,-44}},
        color={0,0,0},
        smooth=Smooth.None));
    connect(MotorAngleSensor.flange,DCmotor_. flange_b1) annotation(Line(points = {{30, -36}, {30, -44}, {20, -44}}, color = {0, 0, 0}, smooth = Smooth.None));
